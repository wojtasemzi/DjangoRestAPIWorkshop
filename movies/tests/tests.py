...
def test_post_movie(client, set_up):
    movies_before = models.Movie.objects.count()
    new_movie = fake_movie_data()
    response = client.post("/movies/", new_movie, format='json')
    assert response.status_code == 201
    assert models.Movie.objects.count() == movies_before + 1
...
def test_get_movie_list(client, set_up):
    ...
    assert models.Movie.objects.count() == len(response.data)
...
def test_delete_movie(client, set_up):
    ...
    movie_ids = [movie.id for movie in models.Movie.objects.all()]
...
def test_update_movie(client, set_up):
    ...
    assert response.status_code == 200
    movie_obj = models.Movie.objects.get(id=movie.id)
